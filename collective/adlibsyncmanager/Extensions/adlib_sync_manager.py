...
def migrate(self):
    ...
    if last_request_successful:
        one_minute_ago = datetime.today() - timedelta(minutes = 2)
    ...
    log_stage_path = "/var/www/teylers-stage/adlib_sync/adlib_sync-stage.log"

    #Create the migrator
    migrator = APIMigrator(self, folder, "", request_type)
    updater = Updater(migrator)
    teylers_migrator = Migrator(updater)
    sync = SyncMechanism(self, teylers_migrator, date, creation_date, request_type, folder, log_stage_path)
