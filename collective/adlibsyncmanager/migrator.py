...
class APIMigrator:
    ...
    def create_object(self, obj):
        ...
        try:
            if hasattr(container, normalized_id) and normalized_id != "":
                ...
                print "%s - Skipped. Already exists: %s" % (timestamp, obj.object_number)
                transaction.commit()
                return True
    ...
    def add_pdfs(self):
        ...
        for obj in self.art_list:
            ...
            if object_item != None:
                ...
                timestamp = datetime.datetime.today().isoformat()
                print "%s - %s: PDF Skipped. Object %s not found." % (timestamp, pdf_id, object_item.object_number)
        ...
        try:
            if hasattr(object_item, 'slideshow'):
                ...
                if not hasattr(slideshow, normalized_id):
                    ...
                    timestamp = datetime.datetime.today().isoformat()
                    print "%s - %s: FOLDER added on %s" %(timestamp, folder_name, object_item.object_number)
                    ...
                    timestamp = datetime.datetime.today().isoformat()
                    print "%s - %s: FOLDER Skipped. Already exists inside slideshow of %s" % (timestamp, folder_name, object_item.object_number)
                    new_folder = slideshow[normalized_id]
                    for img_name in images:
                ...
                timestamp = datetime.datetime.today().isoformat()
                print "%s - %s: Skipped on %s - No slideshow" %(timestamp, folder_name, object_item.object_number)
                return True

        except:
            transaction.abort()
            timestamp = datetime.datetime.today().isoformat()
            print "%s - %s: FOLDER Skipped on %s" %(timestamp, folder_name, object_item.object_number)
            raise
        ...
        try:
            if hasattr(object_item, 'slideshow'):
                ...
                if not hasattr(slideshow, normalized_id):
                    ...
                    timestamp = datetime.datetime.today().isoformat()
                    print "%s - %s: Skipped. Already exists %s" % (timestamp, image_name, object_item.object_number)
                    return True
                ...
                result = True
                timestamp = datetime.datetime.today().isoformat()
                print "%s - %s: Added on %s" %(timestamp, image_name, object_item.object_number)

                return result
            else:
                timestamp = datetime.datetime.today().isoformat()
                print "%s - %s: Skipped on %s" %(timestamp, image_name, object_item.object_number)
                transaction.abort()
                pass
        except:
            transaction.abort()
                ...
                try:
                    if item != None:
                        ...
                        if not ITranslationManager(item).has_translation('en'):
                            transaction.begin()
                            #print "Try to add translation for %s" % (item.object_number)
                                ...
                                for sitem in slideshow:
                                    ...
                                    if self.is_book:
                                        if slideshow[sitem].portal_type == "Image":
                                            ...
                                            if not ITranslationManager(slideshow[sitem]).has_translation('en'):
                                                transaction.begin()
                                                print "Adding translation for book %s inside of book %s" %(sitem, item.object_number)
                                                ...
                                                #print "Added translation for book %s inside of book %s" %(sitem, item.object_number)
                                            else:
                                                nl_folder = slideshow[sitem]
                                                item_n = 0
                                                for fitem in nl_folder:
                            ...
                            #print "Added translation for %s" % (item.object_number)
                        else:
                            print "Translation already created for %s - trying to complete" % (item.object_number)
                            ...
                            #print "Completed translation for %s" % (item.object_number)
        ...
        for obj in self.art_list:
            ...
            if object_data != "":
                ...
                if object_item != None:
                    ...
                    obj_item.object_number = object_data['object_number']
                    ...
                    print "=== Object %s updated ===" %(obj_item.object_number)
    ...
    def fix_all_tags(self):
        ...
        for obj in all_objects:
            try:
                ...
                timestamp = datetime.datetime.today().isoformat()
                print "%s - Reindex object %s" %(timestamp, item.object_number)
                ...
                transaction.abort();
                timestamp = datetime.datetime.today().isoformat()
                print "%s - Skipped object %s" %(timestamp, item.object_number)
                self.skipped_ids.append(item.object_number)
                raise
    ...
    def delete_paintings(self):
        ...
        for obj in container:
            item = container[obj]
            if item.portal_type == "Object":
                if item.object_number in list_objn:
                    container.manage_delObjects([obj])
                    total+=1
                    print total
        print total
                ...
                if obj.portal_type == "Object":
                    ...
                    if object_number != None:
                        ...
                        if priref != "":
                            ...
                            obj.object_number = object_data['object_number']
    ...
    def get_all_ids(self):
        ...
        for obj in container:
            item = container[obj]
            if item.portal_type == "Object":
                objs_numbers.append(item.object_number)
                ...
                if hasattr(item, 'slideshow'):
                    ...
                    if len(slideshow.objectIds()) == 0:
                        try:
                            ...
                            timestamp = datetime.datetime.today().isoformat()
                            print "[%s] Object deleted %s." %(timestamp, item.object_number)
                            total += 1
                        except:
                            timestamp = datetime.datetime.today().isoformat()
                            print "[%s] ERROR. Object not deleted %s." %(timestamp, item.object_number)
                            not_deleted.append(item.object_number)
                            pass
            ...
            for obj in container:
                ...
                if item.portal_type == "Object":
                    ...
                    if review_state != "published":
                        if item.object_number == None:
                            container.manage_delObjects([obj])
                            timestamp = datetime.datetime.today().isoformat()
                            print "[%s] Object deleted [%s] - %s." %(timestamp, item.object_number, obj)
                            total_unpublished += 1
    ...
    def create_zm_object(self, priref, record, create):
        print "Create %s on ZM collection." %(str(priref))
    ...
    def get_zm_collection(self, path):
        xmlFilePath = "/var/dev/collectie-zm-v0/xml/grouped.xml"
        xmlDoc = etree.parse(xmlFilePath)
    ...
    def image_in_list_images(name, images_list):
    ...
    def import_zm_collection(self):
        objects = self.get_zm_collection("/var/dev/collectie-zm-v0/xml/grouped.xml")

        print "GOT ZM COLLECTION"

        list_images = []
        for root, dirnames, filenames in os.walk('/home/rui/zm-collection'):
            for filename in fnmatch.filter(filenames, '*.jpg'):
                list_images.append({"path": os.path.join(root, filename), "name": filename})

        
        image_log = open('/var/dev/collectie-zm-v0/migration/image-log.csv', 'w')
        ...
        obj_log = open('/var/dev/collectie-zm-v0/migration/object-log.csv', 'w')
        ...
        for obj in objects:
            try:
                if len(obj.findall('image_reference')) > 0:
                    ...
                    for rep in obj.findall('image_reference')[0]:
                        if rep.text != None:
                            image_path = rep.text
                            if image_path != None:
                                ...
                                if image_real_path != None:
                                    ...
                                    if priref != None and object_number != None:
                                        ...
                                        result = self.create_zm_object(priref, obj, True)
                                        failed = 1
                    ...
                    total_images = len(obj.findall('image_reference'))
                ...
                print "Object failed"
                timestamp = datetime.datetime.today().isoformat()
                print "[%s] Object failed unexpected %s" %(timestamp, image)
                raise
