...
class SyncMechanism:
    ...
    def fetch_book_api(self, priref, create):
        API_REQ = "http://teylers.adlibhosting.com/wwwopacx/wwwopac.ashx?database=choicebooks&search=priref=%s&xmltype=grouped" % (priref)
    ...
    def fetch_object_api(self, priref, create):
        ...
        if object_temp_data['production_date_start'] == "" and object_temp_data['production_date_end'] == "" and first_record.find('Production_date') != None:
            if first_record.find('Production_date').find('production.date.start.prec') != None:
                ...
                if "fossielen-en-mineralen" in self.folder_path or "fossils-and-minerals" in self.folder_path or self.is_test or object_data['object_number'] == "3014-000" or 'fossiel' in object_data['object_type']:
        ...
        if (("fossielen-en-mineralen" in self.folder_path) or ("fossils-and-minerals" in self.folder_path) or (self.object_type == "fossil") or ('fossiel' in object_data['object_type'])) and object_data["title"] != "":
            object_data["description"] = object_data["title"]

        if (("fossielen-en-mineralen" in self.folder_path) or ("fossils-and-minerals" in self.folder_path) or (self.object_type == "fossil") or ('fossiel' in object_data['object_type'])) and object_data["title"] != "" and object_data['scientific_name'] != "":
        ...
        if (("fossielen-en-mineralen" in self.folder_path) or ("fossils-and-minerals" in self.folder_path) or (object_data['object_number'] == "3014-000") or ('fossiel' in object_data['object_type'])):
            object_data['dating'] = ""
        ...
        if ("Fossils" in self.image_folder) or ("fossielen-en-mineralen" in self.folder_path) or ('fossiel' in object_data['object_type']):
            if first_record.find('title.translation') != None:
                object_data["translated_title"] = first_record.find('title.translation').text
        ...
        if ("Fossils" in self.image_folder) or ("fossielen-en-mineralen" in self.folder_path) or (object_data['object_number'] == "3014-000") or ('fossiel' in object_data['object_type']):
    ...
    def create_items(self, records, path, is_kunst=False):
        ...
        for record in list(records):
            if record.find('object_number') != None or (self.is_book):
                ...
                priref = record.find('priref').text
                if (priref != "24921") or (self.is_book == False):
                    continue
